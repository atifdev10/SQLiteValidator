...
//  SQLiteValidator
//
//  Created by Atif on 21.12.2024.
//
import Foundation
import SwiftDiagnostics
import SwiftSyntax

// Taken from: https://github.com/stackotter/swift-macro-toolkit
...
/// A generic macro error. If you are making a widely used macro I'd encourage you to instead provide more detailed
/// diagnostics through the diagnostics API that macros have access to.
...
/// A way to create rich diagnostics with no unnecessary boilerplate code. Only provide the important details and the
/// rest will be given sensible defaults.
public struct DiagnosticBuilder {
    ...
    /// Adds a replacement fix-it to the diagnostic. If `messageID` is `nil`, the fix-it will inherit the current
    /// `messageID` set via ``DiagnosticBuilder/messageID(_:)`` (or the default messageID of
    /// `"UnknownDomain", "UnknownError"`).
    ...
    /// Build the final diagnostic to be emitted. Defaults will be used for any unset configuration values.
